...
describe('Example Service', () => {
  ...
  describe('/getAll', () => {
    it('should get all the examples', async () => {
      await Promise.all(
        mockExamplesData
          .slice(0, 5)
          .map((item) => service.create(item as unknown as ExampleDocument))
      );
    ...
    it('Should return the paginated data', async () => {
      const examplesData: ExampleDocument[] =
        pagenatedExamplesMockData as unknown as ExampleDocument[];

      await Promise.all(examplesData.map((item) => service.create(item)));

      const { items: examplesDataPaginated }: { items: ExampleDocument[] } = await service.getAll({
        page: 1,
        limit: 5,
        search: '',
      });
      ...
      const { items: nextPageItems }: { items: ExampleDocument[] } = await service.getAll({
        page: 2,
        limit: 5,
        search: '',
      });
  ...
  describe('/findById', () => {
    ...
    it('should throw EntityNotFoundException for invalid id format', async () => {
      ...
      await expect(service.findById(invalidId)).rejects.toThrow(
        'Cast to ObjectId failed for value "invalid-id-format" (type string) at path "_id" for model "Example"'
      );
    });
  ...
  describe('/delete', () => {
    it('should soft delete an example', async () => {
      ...
      const deletedExample: ExampleDocument = await service.delete({
      ...
      await expect(service.findById(createdExample._id.toString())).rejects.toThrow(
        'examples Not Found'
      );
    });
    ...
    it('should throw EntityNotFoundException for invalid id format', async () => {
      ...
      await expect(service.delete({ _id: invalidId })).rejects.toThrow(
        'Cast to ObjectId failed for value "invalid-id-format" (type string) at path "_id" for model "Example"'
      );
    });
  });
  ...
  describe('/update', () => {
    it('should update an example', async () => {
      ...
      const updatedExample: ExampleDocument = await service.update(
        updateData as unknown as ExampleDocument
      );
    ...
    it('should throw EntityNotFoundException for non-existent id', async () => {
      ...
      await expect(service.update(updateData as unknown as ExampleDocument)).rejects.toThrow(
        'examples Not Found'
      );
    });
